...
class LearnedSSLLightning(plReconModel):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx): 
        ...
        # plot zero-filled images for qualitative monitoring
        # - lambda_set: k-space restricted to the lambda subset
        # - all_mask: original acquisition mask (no split into subsets)
        zero_filled_lambda = self.k_to_img_scaled_clipped(
            undersampled_k * input_mask,
            image_scaling,
        )
        zero_filled_inverse = self.k_to_img_scaled_clipped(
            undersampled_k * loss_mask_wo_acs,
            image_scaling,
        )
        zero_filled_all = self.k_to_img_scaled_clipped(
            undersampled_k,
            image_scaling,
        )
        wandb_logger.log_image(
            "train/zero_filled_lambda_set",
            self.split_along_contrasts(zero_filled_lambda[0]),
        )
        wandb_logger.log_image(
            "train/zero_filled_inverse_mask",
            self.split_along_contrasts(zero_filled_inverse[0]),
        )
        wandb_logger.log_image(
            "train/zero_filled_all_mask",
            self.split_along_contrasts(zero_filled_all[0]),
        )
    ...
    def on_validation_batch_end(self, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if plot_images and isinstance(self.logger, WandbLogger):
            ...
            # zero-filled references (use lambda_set vs original acquisition mask)
            input_mask, _ = self.partition_k_space(batch)
            all_mask = batch["mask"].clone()
            if batch["is_self_supervised"].all():
                all_mask = all_mask + batch["loss_mask"]
            zf_lambda_k = batch["undersampled"] * input_mask
            zf_all_k = batch["undersampled"] * all_mask
            ...
            zf_inverse_k = batch["undersampled"] * mask_inverse_w_acs
            zf_lambda_img = k_to_img(zf_lambda_k, coil_dim=2) / scaling_factor
            zf_all_img = k_to_img(zf_all_k, coil_dim=2) / scaling_factor
            zf_inverse_img = k_to_img(zf_inverse_k, coil_dim=2) / scaling_factor
            wandb_logger.log_image(
                f"val_images_zero_filled/zero_filled_lambda_set_{batch_idx}",
                self.split_along_contrasts(zf_lambda_img[0].clip(0, 1)),
            )
            wandb_logger.log_image(
                f"val_images_zero_filled/zero_filled_inverse_mask_{batch_idx}",
                self.split_along_contrasts(zf_inverse_img[0].clip(0, 1)),
            )
            wandb_logger.log_image(
                f"val_images_zero_filled/zero_filled_all_mask_{batch_idx}",
                self.split_along_contrasts(zf_all_img[0].clip(0, 1)),
            )
            wandb_logger.log_image(
    ...
    def on_test_batch_end(self, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        # log zero-filled references during test
        if isinstance(self.logger, WandbLogger):
            wandb_logger = self.logger
            undersampled_k = batch["undersampled"].clone()

            # normalize with the same scaling used for other test visualizations
            scaling_factor = fully_sampled_image.amax((-1, -2), keepdim=True)

            # all/original sampling mask (already unioned in infer_k_space for self-supervised)
            zf_all_img = k_to_img(undersampled_k * mask, coil_dim=2) / scaling_factor
            wandb_logger.log_image(
                f"test/zero_filled_all_mask",
                self.split_along_contrasts(zf_all_img[0].clip(0, 1)),
            )
