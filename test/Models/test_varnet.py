...
def test_varnet_forward(varnet_model):
    with torch.no_grad():
        ...
        mask = (torch.rand(1, 1, 10, 256, 256) > 0.5).to(torch.float32)  # Example mask input
        zf_mask = mask

        output_k = varnet_model.forward(reference_k, mask, zf_mask)
...
def test_varnet_block_forward(unet_model):
    with torch.no_grad():
        ...
        sensetivities = torch.complex(
            torch.randn(1, 1, 4, 256, 256),
            torch.randn(1, 1, 4, 256, 256),
        )
...
def test_varnet_backwards(varnet_model):
    ...
    mask = torch.ones(1, 1, 4, 256, 256, dtype=torch.float32)  # Example mask input
    zf_mask = mask
    ...
    # Snapshot a small subset of parameters to verify an update occurred.
    params = [p for p in varnet_model.parameters() if p.requires_grad]
    before = [p.detach().clone() for p in params[:8]]

    loss = torch.nn.functional.mse_loss(output_k, label)
    loss.backward()

    # Backward should populate at least one finite, non-zero gradient.
    has_grad = any(
    ...
    after = [p.detach() for p in params[:8]]
...
def test_varnet_different_sizes(batch_size, contrasts, height):
    ...
    zf_mask = mask
    output = model(k_space, mask, zf_mask)
